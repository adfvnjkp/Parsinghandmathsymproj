...
def inkml2png(argv):
    if len(argv) < 3:
        print("\n + Usage:", argv[0], " (file|folder) dim padding")
        ...
        if os.path.isfile(argv[1]):
            ...
            if argv[1][-1] != os.sep:
                argv[1] += os.sep
            FILES = glob(argv[1] + os.sep + "*.inkml")

        save_path = "data_test_png"
        #folder_name = argv[1].split(os.sep)[-2]
        if not os.path.isdir(save_path):
            os.mkdir(save_path)

        dim = 28 if len(argv) < 3 else int(argv[2])
        padding = 0 if len(argv) < 4 else int(argv[3])
